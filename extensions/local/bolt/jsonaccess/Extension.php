...
class Extension extends \Bolt\BaseExtension
{
    ...
    public function initialize()
    {
        ...
        $this->config['thumbnail'] = array(
            'width' => isset($this->config['thumbnail']['width']) ? $this->config['thumbnail']['width'] : 320,
            'height' => isset($this->config['thumbnail']['height']) ? $this->config['thumbnail']['height'] : 240,
        );
    ...
    public function json_list(Request $request, $contenttype)
    {
        ...
        } elseif (isset($this->config['contenttypes'][$contenttype]['limit'])) {
            $options['limit'] = intval($this->config['contenttypes'][$contenttype]['limit']);
        }
    ...
    private function clean_item($item, $type = 'list-fields', $thumbsize)
    {
        ...
        $values['excerpt'] = (string) $item->excerpt(100);
        ...
        foreach($item->contenttype['fields'] as $key => $field) {
            if (($field['type'] == 'image' || $field['type'] == 'file') && isset($values[$key])) {
                ...
                // Old style images, that are just 'filename.jpg'
                if (!is_array($values[$key])) {
                    $values[$key] = array('file' => $values[$key]);
                }
    ...
    private function clean_list_item($item)
    {
        ...
        return $this->clean_item($item, 'list-fields', $this->config['thumbnail']);
    ...
    private function clean_full_item($item)
    {
        ...
        $contenttype = $item->contenttype['slug'];
        if (is_array($this->config['contenttypes'][$contenttype]['thumbnail'])) {
            $thumbsize = $this->config['contenttypes'][$contenttype]['thumbnail'];
        } else {
            $thumbsize = $this->config['thumbnail'];
        }

        return $this->clean_item($item, 'item-fields', $thumbsize);
    }
